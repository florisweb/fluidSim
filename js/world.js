...
const World = new class {
	...
	update() {
		...
		this.collisionDetector.resolveNeighbourCollisions(dt); // [26264, 261, 102, 0]
		...
		// this.#deltaPos += this.averageVelocity * dt;
		// if (this.#deltaPos < this.#updateNeighbourRange * .1) return;
		World.collisionDetector.updateNeighbourTable(this.#updateNeighbourRange**2);
		this.#deltaPos = 0;
	}
}
...
// World.particles[1].applyForce(new Vector(-100000000, -100000000));

for (let x = 0; x < World.size.value[0]; x += 10) 
{
	for (let y = 0; y < 100; y += 10) 
	{
		World.particles.push(new Particle({position: new Vector(x + 0, y), radius: .5 + Math.random() * 2}));
	}
}
