...
const World = new class {
	...
	systems = [
		new System({size: new Vector(100, 100)}),
		new System({size: new Vector(50, 100)})
	];
...
for (let x = 0; x < World.systems[0].size.value[0]; x += 10) 
{
	for (let y = 0; y < 100; y += 10) 
	{
		World.systems[0].addParticle(new Particle({position: new Vector(x + 0, y), radius: .5 + Math.random() * 2}));
	}
}
